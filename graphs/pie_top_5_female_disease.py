...
def top_5_female(year,month):
	#LEGENDS
	print("year ",year, "graph and data is generating for female 5  highest")
	#DATA OF 4
	#month basis male nd female of 1 year 
	#specify year dynamicaly
	...
	rog=[]
	data=[]
	for row in cur.execute('select dis_id,sum(female) as female from patient group by dis_id order by female desc limit 5'):
		data.append(row[1])
	...
	#plt.show()
	name='graphs/static/g12_'+str(2000+year)+'.png'
	fig.savefig(name, dpi=100)
	
	plt.clf()#clear privius subplots
	nm='g12_'+str(2000+year)+'.png'
	tableth=['disease','quntity']
	tabletd=[]
	desc="in year "+str(year+2000)+"there are top 5 disease are "+','.join(rog)+" of total patient "+str(sum(data))+"  in Female out of that : \n"
	...
	data=['top 5 Disease in Female',nm,desc,tableth,tabletd]
	print("year ",year, "graph and data is generating for female 5  highest finish")
	return data