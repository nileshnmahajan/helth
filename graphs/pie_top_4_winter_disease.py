...
def top_4_winter(year,month):
	#LEGENDS
	print("year ",year, "graph and data is generating for winter  highest")
	#DATA OF 4
	...
	rog=[]
	data=[]
	for row in cur.execute('select dis_id,sum(male+female) as total from patient where month IN (11,12,1,2) group by dis_id order by total desc limit 4'):
		data.append(row[1])
	...
	name='graphs/static/g11_'+str(2000+year)+'.png'
	
	fig.savefig(name, dpi=100)
	plt.clf()#clear privius subplots
	nm='g11_'+str(2000+year)+'.png'
	...
	data=['top 4 Disease in winter Season',nm,desc,tableth,tabletd]
